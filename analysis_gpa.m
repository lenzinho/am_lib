...
% step 1) load DM3
[file,path,indx] = uigetfile('*.dm3');
Ir = import_dm3([path,file]); Ir = Ir.image_data; Ir=Ir(:,:,1);

% step 2) calculate GPA
[E,W,Ar_g,Pr_g,dPr_g] = gpa(Ir,'rot:field');

% step 3) convert to and plot active strain field
activate_ = @(E) 1./(E+1)-1; 
...
for i = 2:numel(ax); colormap(ax(i),redblue(10)); caxis(ax(i),[-0.4 0.4]); end
...
function [E,W,Ar_g,Pr_g,dPr_g] = gpa(Ir,flag,th,ex_r_,ex_g_)
    ...
    if contains(flag,'interactive') || isempty(ex_g_)
        figure(2); clf; hk = imagesc_(log(abs(Ik).^2)); title('Zoom in on the image; press enter when done.'); 
...
function [dm3struct] = import_dm3( dm3filename )
    global dm3file;
    global endianstyle;
