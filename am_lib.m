...
function [bvk] = get_bvk_model(ip)
        ...
        for p = 1:ip.nshells
            ...
            for i = 1:numel(ip.m{p})
                mp = 3*(abs(ip.m{p}(i))-1)+[1:3]; np = 3*(abs(ip.n{p}(i))-1)+[1:3];
                if (ip.m{p}(i)<0) 
                    D( np,mp ) = D( np,mp ) + B';
                else
                    D( mp,np ) = D( mp,np ) + B;  
                end
                mp = 3*(mn(1)-1)+[1:3]; np = 3*(mn(2)-1)+[1:3]; D( mp,np ) = D( mp,np ) + B;  
            end
...
function [dc]  = generate_bvk_displacement(bvk,ip,uc,nsteps,kpt,amplitude,mode)
    ...
    % define units sqrt( dynamical matrix [Hz^2] = [eV/Ang^2] * [1/amu] ) * hbar = 0.0646555 eV
    units_eV = 0.06465555;

    % load force constants (ignoring self-forces) and define input
    fc = [bvk.fc{:}]; fc = fc([bvk.shell{:}]~=1);
    input = num2cell([fc,kpt(:).',bvk.mass]); [U,hw]=eig(bvk.D(input{:})); 
    hw=sqrt(real(diag(hw)))*units_eV; [hw,inds]=sort(hw); U=U(:,inds);
    ...
    units_THz = 98.22906; dt=(nsteps-1)/(hw(:)*units_THz);
